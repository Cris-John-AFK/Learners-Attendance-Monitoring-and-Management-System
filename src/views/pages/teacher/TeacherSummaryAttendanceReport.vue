...
const maleStudents = computed(() => {
    if (!reportData.value?.students) return [];
    return reportData.value.students.filter((student) => student.gender === 'Male').sort((a, b) => a.name.localeCompare(b.name)); // Alphabetical sort
});

const femaleStudents = computed(() => {
    if (!reportData.value?.students) return [];
    return reportData.value.students.filter((student) => student.gender === 'Female').sort((a, b) => a.name.localeCompare(b.name)); // Alphabetical sort
});
...
const getStudentRemarks = (student) => {
    ...
    const autoGeneratedRemarks = ['Auto-marked absent when session completed', 'Attended class', 'Marked present', 'On time'];
    ...
    reportData.value.days_in_month.forEach((day) => {
        ...
        if (dayDate >= start && dayDate <= end) {
            ...
            if (remarks && remarks !== '-' && !autoGeneratedRemarks.includes(remarks)) {
...
async function loadQuarters() {
    try {
        ...
        try {
            ...
            response = await fetch(`${apiUrl}/api/teachers/${teacherId}/quarters`);
        } catch (error) {
...
onMounted(async () => {
    ...
    if (teacherData?.teacher?.homeroom_section) {
        ...
        const homeroomWithGrade = {
            ...homeroomSection,
            grade_level: homeroomSection.grade_level || homeroomSection.grade?.name || homeroomSection.grade?.grade_name || 'Unknown Grade'
        };
