...
# Replace the assignSection function with one that includes teacher validation
$oldAssignSectionFunction = @'
const assignSection = async (teacher) => {
    console.log('Opening assign section dialog for teacher:', teacher);
    selectedTeacher.value = teacher;
    
    // Filter out sections that already have a homeroom teacher
    try {
        ...
        // Only show sections WITHOUT a homeroom teacher (or where this teacher IS the homeroom teacher)
        const availableSections = allSections.filter(section => {
            ...
            console.log(`Section ${section.name}: homeroom_teacher_id=${section.homeroom_teacher_id}, hasHomeroomTeacher=${!!hasHomeroomTeacher}, isCurrentTeacher=${isCurrentTeacher}`);
            
            return !hasHomeroomTeacher || isCurrentTeacher;
        });
        
        sections.value = availableSections.map(section => ({
            id: Number(section.id),
            name: section.name || `Section ${section.id}`,
            grade: section.grade || section.curriculum_grade?.grade,
            homeroom_teacher_id: section.homeroom_teacher_id
        }));
        
        console.log(`Filtered sections: ${sections.value.length} available (excluding those with homeroom teachers)`);
        console.log('Available sections:', sections.value.map(s => s.name));
    } catch (error) {
...
$content = $content -replace [regex]::Escape($oldAssignSectionFunction), $newAssignSectionFunction

# Write the updated content back to the file
Set-Content -LiteralPath $path -Value $content -Encoding UTF8

Write-Output "Fixed homeroom assignment to show only compatible sections based on teacher type"
